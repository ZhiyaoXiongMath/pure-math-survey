...
Give a full statement one local home in each manuscript. Before it, introduce the question or motivation; in the environment, state the precise mathematics; afterward, explain, prove, compare or apply it. Do not repeat the complete hypotheses and conclusion in adjacent prose and then present them again as a theorem. A brief preview or a later reminder of a condition needed for use is legitimate. Necessary restatement across independently readable documents is also legitimate; this is not global deduplication across the series.

For example, introduce why an equivalence matters, state it once, then explain one direction or the role of an assumption. Do not write the full equivalence with all quantifiers immediately before the formal theorem. Consolidate repeated boundary warnings at the relevant statement or example, but keep material qualifications beside the claims they govern. Reject automatic near-duplicate deletion: similar formulas may express genuinely different assumptions or conclusions.
...
Display formulas according to mathematical role and length; number those that need reference. Prefer `align`, `aligned`, `gathered` or `cases` for structured expressions. Keep short incidental notation inline. Do not rely on global `\sloppy`, negative spacing, large `samepage` blocks or wholesale font reduction to hide defects. Check long formulas, citation spacing and numbering in the actual rendered pages; local issues call for local fixes unless a common defect is reproduced. Also read notation definitions after macro expansion: a sentence meant to introduce a symbol must not print the same expanded expression on both sides. This is a semantic copyedit, not a rule forbidding legitimate identities. Compilation success or a text-similarity score cannot perform it.
