...
A single-topic request selects V concise. An explicit series or end-to-end suite selects I–V concise. The ten templates are a catalog; standard and paired editions require a request. The manifest must explicitly record the selected documents, even though the legacy validator retains its ten-document compatibility fallback.
