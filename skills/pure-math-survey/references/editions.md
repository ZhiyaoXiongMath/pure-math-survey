...
Every requested edition must state its own selected principal conclusions in its introduction. Concise may select a narrower coherent scope, but may not replace selected conclusions with names, citations or a roadmap. Standard may not defer the knowledge to the body simply because the body is longer. Necessary assumptions, quantifiers, exceptions and source qualifications survive compression. The body reference supplements rather than supplies missing introduction content.

Both V editions use [the v6 benchmark](part-v-benchmark.md) as the same quality floor within their declared scope. Concise is a focused completed review, not a synopsis; standard deepens the same thread through supported comparisons, revealing derivations, hypotheses and boundary discussion, not arbitrary additional topics. Neither edition must reproduce v6's exact page count, section count, mathematical subject or theorem count.
