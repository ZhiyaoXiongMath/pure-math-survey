...
Read [template maintenance](references/template-maintenance.md) and [layout and build](references/layout-and-build.md). Run `scripts/validate_assets.py` and `scripts/build_checks.py`, then inspect renders. Development unit tests are maintained separately from this clean installation. Current-style regression must use `assets/templates/math-review.sty`; frozen reproduction must use the sample's bundled style in a separate target. Test with deliberately different dependencies so a stale copy cannot pass unnoticed. Preserve frozen evidence unchanged and date new verification separately.
