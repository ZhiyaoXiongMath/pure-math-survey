...
LEGACY_SCHEMA_VERSION = "0.8.0"
EDITIONS = {"concise", "standard"}
LEGACY_COMBINATIONS = {(part, edition) for part in range(1, 5) for edition in EDITIONS}
COMBINATIONS = LEGACY_COMBINATIONS | {(5, edition) for edition in EDITIONS}
MAP_FIELDS = {
...
REQUIRED_AUDITS = {"source_identity", "statement_verification", "proof_depth",
                   ...
                   "edition_semantic_consistency", "frontier_status", "build_all_eight",
                   "visual_all_eight", "archive_integrity"}
...
def requested_combinations(manifest: dict, errors: list[str]) -> set[tuple[int, str]]:
    """Default by explicit schema; never infer the request from existing files."""
    legacy = manifest.get("schema_version") == LEGACY_SCHEMA_VERSION
    allowed = LEGACY_COMBINATIONS if legacy else COMBINATIONS
    ...
    for number, item in enumerate(selection, 1):
        ...
        if (type(part) is not int or part not in range(1, 5 if legacy else 6) or
...
def validate(root: Path, template_mode: bool = False) -> tuple[list[str], list[str]]:
    ...
    if manifest.get("schema_version") not in (LEGACY_SCHEMA_VERSION, SCHEMA_VERSION):
        return [f"schema_version must be {LEGACY_SCHEMA_VERSION} or {SCHEMA_VERSION}"], notes
    ...
    if seen != requested or len(documents) != len(requested):
        contract = ("requested_documents" if "requested_documents" in manifest
                    else ("the default eight part/edition combinations" if legacy
    ...
    map_fields = MAP_FIELDS if legacy else MAP_FIELDS | INTEGRATED_MAP_FIELDS
    mappings = read_csv(root / "publication-map.csv", map_fields, errors)
    if legacy:
        # Read-only compatibility: absent V placements are unrequested.
        for row in mappings:
            for edition in EDITIONS:
                row.setdefault(f"integrated_{edition}_file", "")
                row.setdefault(f"integrated_{edition}_label", "")
                row.setdefault(f"integrated_{edition}_treatment", "NOT_REQUESTED")
    if not mappings:
    ...
    if audit.is_file():
        ...
        for row in rows:
            ...
            if status in PASS_STATUSES and not (row.get("evidence") or "").strip():
                ...
                (notes if template_mode else errors).append(message)
        missing_checks = (REQUIRED_AUDITS - {"build_all_eight", "visual_all_eight"}) - seen_checks
        if proof_explanation_requested:
            missing_checks |= PART_III_AUDITS - seen_checks
        for requested_check in REQUESTED_DOCUMENT_AUDITS:
            legacy_check = requested_check.replace("requested_documents", "all_eight")
            if requested_check not in seen_checks and not (requested == LEGACY_COMBINATIONS and legacy_check in seen_checks):
                missing_checks.add(requested_check)
        if missing_checks:
